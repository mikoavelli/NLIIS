...
class ChangeHandler(FileSystemEventHandler):
    ...
    def on_any_event(self, event):
        """Signal that a meaningful change occurred for any .html file event."""
        ...
        is_html_event = False
        if hasattr(event, 'src_path') and event.src_path.endswith('.html'):
            is_html_event = True
        if hasattr(event, 'dest_path') and event.dest_path.endswith('.html'):
            is_html_event = True

        if not event.is_directory and is_html_event:
